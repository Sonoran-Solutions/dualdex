 ...
 *   ignored. This is the tier issue #86 (Caveat mode) will populate. **On current `main`
 *   there are zero of these, and the census does not fake any**: the production policy has
 *   no notion of an ignored mechanic yet, so nothing can be classified here. When #86 lands,
 *   it feeds its real production outcome into this same classification and no other part of
 *   the census has to change.
 ...
 * reader can always get from the census back to the policy decision that produced it.
 */
 ...
 * adds no capability opinion of its own. This is the single place issue #86 will extend (by
 * feeding it the split `blocking` / `ignoredMechanics` lists), and the census schema already
 * has the `CAVEATED_ESTIMATE` slot waiting.
 */
