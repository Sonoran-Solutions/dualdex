...
| `scenarios/60-golden-a-neutral.txt` | **PASSES.** Gap C4d official-ROM golden A: neutral ordinary Tackle damage (see `evidence/`) |
| `scenarios/61-golden-c-stat-stage.txt` | **PASSES.** Gap C4d official-ROM golden C: live non-neutral Defense stage via Leer, then Scratch |
| `scenarios/62-golden-b-stab-grind.txt` | **DEVELOPER AID, exits non-zero by design.** Grinds the starter to level 6 (Razor Leaf) and captures the first Razor Leaf hit. It tolerates `hunt` timeouts and move-not-known errors while leveling, so its exit status is not the result; the captured hit in `evidence/golden-b-stab-type-razor-leaf.log` is |
